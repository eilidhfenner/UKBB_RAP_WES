...
combined %>% count(SZ_associated_CNV_carrier) 
...
combined %>% count(SZ_enriched_CNV_carrier) 
...
combined %>% count(CNV_carrier_15q_only) 
...
EU_ancs_w_g=subset(EU_ancs, !is.na(EU_ancs$g_MAIN_RT_TMTB_NM_PM_zscore_outliersrm)) 
backup=EU_ancs_w_g
```
...
multivar_model <- lm(g_MAIN_RT_TMTB_NM_PM_zscore_outliersrm ~ 
                       HC_PTV_AC5orless_pLI_0.9_total + 
