...
a %>% count(HC_PTV_AC5orless_pLI_0.9_total)
...
combined_summary <- bind_rows(
  summaryHCPTVS %>% rename(n_variants = HC_PTV_AC5orless_pLI_0.9_total),
...
combined_summary$variant_class <- factor(
  ...
  levels = c("HC_PTVs", "Deleterious missense", "Synonymous")
)
...
summary_ancs <- all_ancs %>%
  ...
  summarise(
    #Exome wide
    mean_EW_HC_PTVs = mean(HC_PTV_AC5orless_exome_wide_total, na.rm = TRUE),
    se_EW_HC_PTVs = sd(HC_PTV_AC5orless_exome_wide_total, na.rm = TRUE) / sqrt(n()),
    ...
    # LoFi
    mean_LoFi_HC_PTVs = mean(HC_PTV_AC5orless_pLI_0.9_total, na.rm = TRUE),
    se_LoFi_HC_PTVs = sd(HC_PTV_AC5orless_pLI_0.9_total, na.rm = TRUE) / sqrt(n()),
...
var_types<-list('HC_PTV', 'REVEL75_Miss')
...
```{r}
write.csv(results, 'results_g_regressions_AC5orless_diagnosesrm_incSE_crossancs.csv')
system('dx upload results_g_regressions_AC5orless_diagnosesrm_incSE_crossancs.csv --destination ./Variant_counts/Gene_sets/')
```
