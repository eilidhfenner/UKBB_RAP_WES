...
This script reads in phenotypic data and tables of counts per gene set, and then runs regressions on these data. This script runs for <1 hour minutes on a mem1_ssd1_v2_x8 RStudio instance, costing £0.20.
# Load libraries
```{r}
library(data.table)
library(dplyr)
...
```{r}
system('dx download ./phenotype_cohorts/participation_diagnoses_participant.csv')
diagnoses=read.csv('participation_diagnoses_participant.csv', header=T)

phen %>% count(p130875)
phen$SZ<-T
phen$SZ[phen$p130875=='']<-F
phen %>% count(SZ)

phen %>% count(p130951)
phen$intellectual_disability<-T
phen$intellectual_disability[phen$p130951=='' &
                               phen$p130953=='' &
                               phen$p130955==''&
                               phen$p130959==''&
                               phen$p130961=='']<-F
phen %>% count(intellectual_disability)

phen %>% count(p130971)
phen$ASD<-T
phen$ASD[phen$p130971=='']<-F
phen %>% count(ASD)
...
sets <- list('exome_wide',
             ...
             'SCHEMA_FDR_US', 'LoFi_and_SCHEMA_FDR_US','LoFi_minus_SCHEMA_FDR_US')
...
##### FI
```{r}
dependent_var<-list('FI_AC1_n_correct_answers_Zscore', 
                    'SDS_online1_n_correct_cons_attempts_Zscore_outliersrm' , 
                    'TMTA_online1_time_to_complete_numeric_path_Zscore_outliersrm')

EU_ancs=subset(EU_ancs, !is.na(EU_ancs$g_MAIN_RT_TMTB_NM_PM_zscore_outliersrm))

```
##### Just usual covariates
```{r}
var_types<-list('PTV', 'REVEL75_Miss')
...
for (dep_var in dependent_var) {
  ...
  for (var in var_types) {
    ...
    for (set in sets) {
      independent_var <- paste0(var, '_AC', AC, '_', set, '_total')
      formula <- as.formula(paste(dep_var, "~", independent_var, "+", covars_Eu_anc_NS, "+ g_MAIN_RT_TMTB_NM_PM_zscore_outliersrm"))
      model <- lm(formula, data = EU_ancs) 
      tidy_model <- tidy(model)
      ...
      if (nrow(tidy_model) > 0) {
        ...
        conf_int <- confint(model)[2, ] 
        p_value <- tidy_model$p.value[2]
        with_phen=subset(EU_ancs, !is.na(EU_ancs[[dep_var]]) & !is.na(EU_ancs$g_MAIN_RT_TMTB_NM_PM_zscore_outliersrm))
        n_people<-nrow(with_phen)
        n_vars <- sum(with_phen[[independent_var]])
        anc_set<-'EUR_only_covar_g_MAIN_RT_TMTB_NM_PM_zscore_outliersrm'
        # Add to results data frame
        results <- rbind(results, data.frame(
          gene_set = set,
          dep_var = dep_var,
...
for (dep_var in dependent_var) {
  ...
  for (var in var_types) {
    ...
    for (set in sets) {
      independent_var <- paste0(var, '_AC', AC, '_', set, '_total')
      formula <- as.formula(paste(dep_var, "~", independent_var, "+", covars_Eu_anc_syn, '+ g_MAIN_RT_TMTB_NM_PM_zscore_outliersrm'))
      model <- lm(formula, data = EU_ancs) 
      tidy_model <- tidy(model)
