...
chromosomes<-1:22
variant_classes<-list('PTV', 'HC_PTV', 'REVEL75_Miss', 'synonymous')
...
## LoFtol and SCHEMA FDR5 
```{r}
setname='LoFtol_and_SCHEMA_FDR_US'
set=subset(sets, sets$nonconstrained_SCHEMA_FDR5_gene==T) 
list=set$gene_id
...
## Brain expressed and SCHEMA 
```{r}
#sets$BE_SCHEMA_FDR5_gene<-F
#sets$BE_SCHEMA_FDR5_gene[sets$Brain_expressed==T & sets$SCHEMA_FDR5_gene==T] <- T
#sets %>% count(BE_SCHEMA_FDR5_gene) 

setname='BE_and_SCHEMA_FDR_US'
set=subset(sets, sets$BE_SCHEMA_FDR5_gene==T) 
# Filter to only genes with a gene id (as this is what we'll match on)
list=set$gene_id
...
## Brain expressed minus schema
```{r}
#sets$BE_NOT_SCHEMA_FDR5_gene<-F
#sets$BE_NOT_SCHEMA_FDR5_gene[sets$Brain_expressed==T & is.na(sets$SCHEMA_FDR5_gene)] <- T
#sets %>% count(BE_NOT_SCHEMA_FDR5_gene) 

setname='BE_minus_SCHEMA_FDR_US'
set=subset(sets, sets$BE_NOT_SCHEMA_FDR5_gene==T) 
# Filter to only genes with a gene id (as this is what we'll match on)
list=set$gene_id
